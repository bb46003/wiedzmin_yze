...
static _fromStatusEffectSync(statusId, options = {}) {
  ...
  if (!status)
    throw new Error(`Invalid status ID "${statusId}" provided to ActiveEffect#fromStatusEffect`);
  ...
  static _configureStatusEffects() {
    ...
    for (const [id, data] of Object.entries(wiedzmin_yze.config.CONDITIONS)) {
  	...
  	static async _preCreateOperation(documents, operation, user) {
		...
		await wiedzmin_yze.models.Condition._preCreateConditions(documents, operation);
	}
